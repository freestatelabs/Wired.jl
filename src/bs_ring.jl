...
@views function biotsavart!(B::AbstractArray, nodes::AbstractArray, rings::AbstractArray{CircularRing}; 
    ...
    B_ = zeros(datatype, Nnodes,3)     # Define separately for reasons below
    r = zeros(datatype, Nnodes)
    alpha = zeros(datatype, Nnodes)
    a2 = zeros(datatype, Nnodes)
    beta = zeros(datatype, Nnodes)
    k2 = zeros(datatype, Nnodes)
    E = zeros(datatype, Nnodes)
    K = zeros(datatype, Nnodes)
    Jdensity_correction = zeros(datatype, Nnodes)
...
"""
function biotsavart(nodes::AbstractArray, rings::AbstractArray{CircularRing}; mu_r=1.0, errmax=1e-16, datatype=Float64)

    B = zeros(datatype, size(nodes))
...
function biotsavart(nodes::AbstractArray, rect::AbstractArray{RectangularRing}; 
                        mu_r=1.0, Nmin=2, errmax=1e-8, Nt::Integer=0, datatype=Float64)
...
"""
function bfield(nodes::AbstractArray, rings::Vector{<:Ring}; mu_r=1.0, Nmin=2, errmax=1e-8, Nt=0, datatype=Float64)
    ...
    for it = 1:Nt 
        if isa(rings, Vector{CircularRing})
            @views tasks[it] = Threads.@spawn biotsavart(nodes, rings[threadindices(it, Nt, Ns)]; mu_r=mu_r, errmax=errmax, datatype=datatype)
        else 
            @views tasks[it] = Threads.@spawn biotsavart(nodes, rings[threadindices(it, Nt, Ns)]; mu_r=mu_r, errmax=errmax, Nmin=Nmin, datatype=datatype)
        end
    end
